        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            for w in range(n_steps):
                ...
                for i_robot in range(nr_robots):
                    ...
                    if params.ROLLOUT_FABRICS:
                        # rollouts generate from c++ controller:
                        data = np.concatenate((np.zeros(9),
                                               np.zeros(9),
                                               np.array([arguments_robot["weight_goal_0"]]),
                                               arguments_robot["x_goal_0"]))
                        msg = map(str, data)
                        msg = ' '.join(msg)
                        sock.sendall(msg.encode())

                        data = sock.recv(1024)
                        action_rollout = np.fromstring(data.decode(), dtype=float, sep=' ')
                        print("action_rollout:", action_rollout)
