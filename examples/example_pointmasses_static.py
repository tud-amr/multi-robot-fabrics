...
"""
Fabrics example with two point-mass robots and two static obstacles where the robots try to cross 
To make sure that they try to avoid each other, a static obstacle is seen by the planner at the position of the other robot
"""
...
def set_planner_point(goal: GoalComposition, n_obstacles: int = 2, degrees_of_freedom: int = 7, obstacle_resolution = 1):
...
def run_point_example(n_steps=1000, render=True):
    ...
    planner_point = set_planner_point(goal, n_obstacles = n_robots-1 + n_static_obstacles)
    ...
    for _ in range(n_steps):
        ...
        pos_robot0 = ob_robot["joint_state"]["position"][0:3]
        pos_robot1 = ob_robot1["joint_state"]["position"][0:3]
        pos_robot2 = ob_robot2["joint_state"]["position"][0:3]
        pos_robot3 = ob_robot3["joint_state"]["position"][0:3]


        robots = ['robot_0', 'robot_1', 'robot_2', 'robot_3']
        pos_robots = []
        for robot in robots:
            pos_robots.append(ob[robot]["joint_state"]["position"][0:3])



        for i, robot in enumerate(robots):
            pos_obs = [obst['position'][0:3] for obst in list(ob[robot]['FullSensor']['obstacles'].values())] + [pos for j,pos in enumerate(pos_robots) if j!=i]
            radius_obs = [obst['size'] for obst in list(ob[robot]['FullSensor']['obstacles'].values())] + [r for j,r in enumerate(r_robots) if j!=i]
            ...
            action[idx_start:idx_end] = planner_point.compute_action(
                ...
                x_goal_0=goal_robots[i],#ob_robot['FullSensor']['goals'][0][0][0:2],
